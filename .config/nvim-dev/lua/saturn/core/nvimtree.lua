...
function M.setup()
  ...
  if saturn.builtin.nvimtree._setup_called then
    Log:debug "ignoring repeated setup call for nvim-tree, see kyazdani42/nvim-tree.lua#1308"
    return
  end

  saturn.builtin.which_key.mappings["e"] = { "<cmd>NvimTreeToggle<CR>", "Explorer" }
  saturn.builtin.nvimtree._setup_called = true

  -- Implicitly update nvim-tree when project module is active
  if saturn.builtin.project.active then
    saturn.builtin.nvimtree.setup.respect_buf_cwd = true
    saturn.builtin.nvimtree.setup.update_cwd = true
    saturn.builtin.nvimtree.setup.update_focused_file = { enable = true, update_cwd = true }
  end
  ...
  -- Add useful keymaps
  if #saturn.builtin.nvimtree.setup.view.mappings.list == 0 then
    saturn.builtin.nvimtree.setup.view.mappings.list = {
  ...
  nvim_tree.setup(saturn.builtin.nvimtree.setup)

  if saturn.builtin.nvimtree.on_config_done then
    saturn.builtin.nvimtree.on_config_done(nvim_tree)
  end
end
